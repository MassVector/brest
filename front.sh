...
function mountISO {
    sudo mount /iso/OS.iso /iso/repo/smolensk;
    sudo mount /iso/OS-upd.iso /iso/repo/smolensk-upd;
    sudo mount /iso/OS-dev.iso /iso/repo/smolensk-dev;
    sudo mount /iso/brest.iso /iso/repo/brest;
...
case "$STATE" in
    ...
    sudo mkdir /iso/repo/{smolensk,smolensk-dev,smolensk-upd,brest} -p;
...
deb file:///iso/repo/smolensk smolensk contrib main non-free
deb file:///iso/repo/smolensk-upd smolensk contrib main non-free
deb file:///iso/repo/smolensk-dev smolensk contrib main non-free
...
EOF
    ...
    sudo apt install -y astra-freeipa-client brestcloud-ipa;
    ...
    echo "Сервер введен в домен FreeIPA";
    sleep 5;
